...
describe('#validate()', () => {
  ...
  it('should reject not if http event is present and stage contains invalid chars and IGNORE_INVALID_CHAR set', () => {
    const invalidOptions = {
      stage: 'my@stage',
      region: 'us-east-1',
    };
    ...
    process.env.IGNORE_INVALID_CHAR = '*';
    expect(() => awsCompileApigEvents.validate()).to.not.throw(
      serverless.classes.Error,
      [
        'Invalid stage name my@stage: it should contains only [-_a-zA-Z0-9]',
        'for AWS provider if http event are present',
